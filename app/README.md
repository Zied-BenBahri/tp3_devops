...
```bash
# Build image from repo root using the moved Dockerfile
# (note: Dockerfile is under infra/docker now)
...
- `app/index.js` — HTTP server using Node's built-in `http` module
- `app/test.js` — self-test that starts the server on an ephemeral port and validates the response
- Root `package.json` — start and test scripts point to `app/`
- `infra/docker/Dockerfile` — container build instructions
- `infra/docker/.dockerignore` — excludes dev files from Docker context
